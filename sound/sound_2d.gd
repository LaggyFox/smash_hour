...
func play(sound_resource: SoundResourceBase, pos: Vector2) -> void:
	assert(sound_resource.is_2d_sound(), "called a normal sound with 2D sound player")
	...
	if sound_resource is SoundResource:
		play_single(sound_resource as SoundResource, pos)
	else:
		play_random(sound_resource as MultiSoundResource, pos)

func play_single(sound_resource: SoundResource, pos: Vector2, db_offset: float = 0.0) -> void:
	...
	sound_player.attenuation = sr.attenuation
	sound_player.max_distance = sr.max_distance
...
func play_random(sound_resource: MultiSoundResource, pos: Vector2) -> void:
	play_single(sound_resource.get_random_sound(), pos, sound_resource.global_db_offset)
